...
extension HistoryView {
	...
	@objc private func deleteAll(gesture: UIGestureRecognizer) {
		...
		let controller = UIAlertController(title: "Be sure to delete all files？", message: nil, preferredStyle: .alert)
	...
	private func refreshNumber() {
		...
		self.numbersLabel?.text = "file：\(index + 1) / \(self.dataSource.count)"
	}
	...
	@objc private func didClickButton(sender: UIButton) {
		...
		if sender.tag == 0 {
			...
			let controller = UIAlertController(title: "Be sure to delete\(item.time) \(item.name)？", message: nil, preferredStyle: .alert)
			controller.addAction(UIAlertAction(title: "delete", style: .destructive, handler: { (_) in
